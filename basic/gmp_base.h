...
// Basic definition of constant values.
...
#ifdef GMP_PTRADDR_T
typedef EM_PTRADDR_T gmp_ptraddr_t;
#else
typedef uint32_t gmp_ptraddr_t;
#define gmp_ptraddr_s ((4))
#endif // GMP_PTRADDR_T

// container of a offset of address
// generally is a signed type
#ifdef GMP_PTRDIFF_T
typedef GMP_PTRDIFF_T gmp_ptrdiff_t;
#else
typedef int32_t gmp_ptrdiff_t;
#define gmp_ptrdiff_s ((4))
#endif // GMP_PTRDIFF_T
...
// status type
#ifdef GMP_STATUS_T
typedef GMP_STATUS_T gmp_status_t;
#else
typedef uint_fast16_t gmp_status_t;
...
#define GMP_STATUS_WARNING_TYPE     (0x0001)
#define GMP_STATUS_ERROR_TYPE       (0x8001)

// This area to define all warnings or infos.
#define GMP_STATUS_OK				(0x0000)
#define GMP_STATUS_GENERAL_WARN     (GMP_STATUS_WARNING_TYPE + 0)
#define GMP_STATUS_GENERAL_ERROR	(GMP_STATUS_ERROR_TYPE + 0)
#define GMP_STATUS_WARN_PRINT       (GMP_STATUS_WARNING_TYPE + 1)
...
// in BSP files or other source files.
	...
	// Debug log default print function
	// @param content: the debug log to be printed.
	// @param len    : the length of the content.
	// @retval       : This funciton returns nothing. This function should designed as nothrow function.
	...
	// this function would be called, when gmp_abort() function is invoked.
	// That is, the time when the GMP library meet fatal error, and can't resolve.
	// The function would be the last function to be called.
	// @param null
	// @retval null
...
// System basic function which was been defined by GMP library
	...
	// Debug formatted string print function
	...
	// This function will be called when GMP meets a fatal error
	...
	// This macro function provide a easy-using asert for GMP library.
	// This macro would be called when assert is triggered.
#ifdef GMP_DEBUG
#define gmp_assert(expression) (void)(                                                 \
            (!!(expression)) ||                                                        \
            (gmp_assert_print((const gmp_data_t*)(#expression), (__FILE__), (gmp_size_t)(__LINE__)))      \
        )
#else
...
// There're several functions user should call them manually.
	
	// This function will startup the whole library in order.
